...
Pinky::Pinky(float x, float y, float speed, float radius, std::shared_ptr<Maze> maze, raylib::Color color, std::shared_ptr<PacMan> pacman, gamePtr game,Texture2D pinkyTexture)
    : x_(x), y_(y), speed_(speed), radius_(radius), maze_(maze), pacman_(pacman), color_(color), game_(game),pinkyTexture_(pinkyTexture)
{
    startingX_ = x_;
    startingY_ = y_;

    // Add an initial state to the character state manager (Characters class)
    auto initialState = std::make_shared<EnemyChaseState>(x_, y_, speed_, radius_, maze_, color_, pacman_,pinkyTexture_);
